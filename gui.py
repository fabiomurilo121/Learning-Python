...
def searchTask():
    try:
        ...
        for i in f:  # make a list to show number of tasks for each specific day ex: 0)task 1 | 1)task2 and so on
            ...
            if internalList[0] == spinDaySearch.get() and internalList[1] == spinMonthSearch.get() and internalList[
                2] == spinYearSearch.get():
...
def updateTask():
    try:
        ...
        for i in myfile:
            ...
            if internalList[0] == spinDayUpdate.get() and internalList[1] == spinMonthUpdate.get() and internalList[
                2] == spinYearUpdate.get():
                tempList.append(i)
                tempList.append('\n')
        ...
        def saveUpdatedTask():
            tempList[int(
                spinUpdateTask.get())] = spinDayUpdate.get() + ',' + spinMonthUpdate.get() + ',' + spinYearUpdate.get() + ',' + updateEntry.get()
